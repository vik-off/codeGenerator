...
class JackItemController extends Controller{
	
	const DEFAULT_VIEW = 1;
	const TPL_PATH = 'JackItem/';
	...
	public function display_list($params = array()){
		...
		$collection = new JackItemCollection();
		$variables = array(
	...
	public function display_view($params = array()){
		...
		$instanceId = getVar($params[0], 0, 'int');
			...
			$variables = JackItem::Load($instanceId)->GetAllFieldsPrepared();
			FrontendViewer::get()
				->setTitle('Детально')
	...
	public function admin_display_new($params = array()){
		...
		$variables = array_merge($_POST, array(
			'instanceId' => 0,
			'pageTitle'  => $pageTitle,
			'validation' => JackItem::Create()->getValidator()->getJsRules(),
		));
	...
	public function admin_display_edit($params = array()){
		...
		$instanceId = getVar($params[0], 0 ,'int');
		$instance = JackItem::Load($instanceId);
	...
	public function action_save($params = array()){
		...
		$instance = new JackItem($instanceId);
	...
	public function action_delete($params = array()){
		...
		$this->setRedirectUrl('admin/content/jack-item/list');
